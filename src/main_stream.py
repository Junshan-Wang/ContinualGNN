...
def train(data, model, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(len(nodes) // args.batch_size):
            ...
            batch_labels = torch.LongTensor(data.labels[np.array(batch_nodes)])
...
def run(args, t):
    ...
    if t > 0:
        ewc_model = EWC(sage, args.ewc_lambda, args.ewc_type)
        ...
        ewc_model.register_ewc_params(important_nodes_list, torch.LongTensor(data.labels[important_nodes_list]))
    else:
        if args.memory_size > 0:
    ...
    # Memory save
    if args.memory_size > 0:
        train_output = sage.forward(data.train_nodes).data.numpy()
...
def evaluate(args, t):
    ...
    valid_output = sage.forward(valid_nodes).data.numpy().argmax(axis=1)
